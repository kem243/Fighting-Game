            ...
            if !fighter.char_state.isMoving() {
                match event {
                        ...
                        Event::KeyDown{keycode: Some(k), repeat:false, ..} => {
                            if ((fighter.char_state.state == animation::sprites::State::Jump || fighter.char_state.state == animation::sprites::State::FJump) && fighter.char_state.jump_count == fighter.jumps) 
                            && k == &Keycode::W {
                                println!("Do nothing");
                            } else{
                                fighter.char_state.reset_current_frame(); // reset frames to 0 every click
                                match k {
                                    Keycode::W => { input::movement::jump(fighter); fighter.char_state.increment_jump_count(); }, // jump                                                                                     
